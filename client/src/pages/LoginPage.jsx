...
const LoginPage = () => {
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      {/* ------left-------- */}
      <img src={assets.logo_big} alt="" className='h-48' />

      {/* -----right------- */}

      <form onSubmit={onSubmitHandler} action="" className='border-2 bg-white/8 text-white border-gary-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
...

        {
          currState === "Sign up" && isDataSubmitted && (
            <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
             rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2' placeholder='provide a short bio...'></textarea>
          )
        }
